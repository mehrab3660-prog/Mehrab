...
همچنین هر کاربری که با ربات پیام رد و بدل کنه (آیدی عددی، یوزرنیم و متن
پیام‌هاش) در یک پایگاه‌داده‌ی SQLite محلی (`bot_data.db`) ثبت می‌شه. فقط
ادمین (کسی که آیدی عددیش در `ADMIN_ID` تنظیم شده) به دستورات زیر دسترسی
داره:
